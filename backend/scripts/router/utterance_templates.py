...
ROUTING_TEMPLATES: dict[str, list[str]] = {
    ...
    "eda.distribution_overview": [
        "看看{dataset_id}这批数据的分布",
        "{dataset_id}整体的分布情况怎么样",
        "这批数据的分布大致什么样",
        "{dataset_id}各列分布总览一下",
        "看看{dataset_id}的数据都怎么分布",
        "这批数据分布情况给我说说",
        "{dataset_id}数值列和类别列的分布概况",
        "整体看一下{dataset_id}的分布",
        "这批数据大致长什么样，分布如何",
        "{dataset_id}的分布总览",
    ],
    "eda.correlation": [
    ...
    # ---- connector（4，2026-09-24 新增，补齐模型标签空间）----
    "connector.list": [
        "我配了哪些数据库连接",
        "列出已配置的外部数据库",
        "看看我接过的数据源都有哪些",
    ...
    "connector.tables": [
        "看看{connector_id}下有哪些表",
        "列出{connector_id}里的所有表",
        "{connector_id}这个数据源里有什么表",
        "看看{connector_id}里有哪些表",
        "列出{connector_id}的所有表",
        "查一下{connector_id}里可用的表",
        "{connector_id}下面都有哪些表",
        "列出{connector_id}的表清单",
        "看看{connector_id}这个库里有什么",
        "{connector_id}的表结构列一下",
    ],
    "connector.preview": [
        "预览一下{connector_id}里的{table}表",
        "看看{connector_id}的{table}表前几行长什么样",
        "抽样看下{connector_id}里{table}表的数据",
        "预览{connector_id}的{table}表内容",
        "{connector_id}的{table}表数据先看一眼",
        "看看{connector_id}里{table}表的实际数据",
        "预览一下{connector_id}这张{table}表",
        "抽查几条{connector_id}里{table}表的数据",
        "看看{connector_id}的{table}表里的内容",
        "预览{connector_id}数据源的{table}表",
    ],
    "connector.import": [
        "把{connector_id}里的{table}表导入成数据集",
        "从{connector_id}把{table}表导进来",
        "把{connector_id}的{table}表导入平台",
        "导入{connector_id}数据源的{table}表",
        "把{connector_id}里的{table}表拉进来分析",
        "从{connector_id}导入{table}表",
        "把{connector_id}的{table}表导成数据集",
        "导入{connector_id}的{table}这张表",
        "把{connector_id}的{table}表接入平台",
        "从{connector_id}抽取{table}表",
    ],
}
